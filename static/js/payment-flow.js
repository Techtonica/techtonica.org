...
window.showSuccessPage = function (jobTitle, company) {
  ...
  document
    .getElementById("post-another-job")
  ...
  try {
    ...
    for (const word of words) {
      ...
      const response = await fetch(
        `https://www.purgomalum.com/service/containsprofanity?text=${encodedWord}`,
      );
...
window.validateForm = async function () {
  ...
  if (!referralSelected) {
    ...
    if (!firstInvalidField)
      firstInvalidField = document.getElementById("radio-wrapper");
  } else {
  ...
  if (!isValid) {
    if (firstInvalidField) {
      ...
      window.showError(
        "Please fill in all required fields and accept the Code of Conduct.",
      );
    }
    return false;
  }
  ...
  for (const input of textInputs) {
    ...
    if (text) {
      ...
      if (profanityCheck.containsProfanity) {
        ...
        window.showError(
          `Potentially inappropriate word "${profanityCheck.text}" detected in ${fieldLabel}. Please revise your content.`,
        );
